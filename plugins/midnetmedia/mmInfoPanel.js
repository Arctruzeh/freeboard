(function () {
    ...
    freeboard.addStyle('.ip-container', 'width:300px;height:240px;font-family:"Source Sans Pro", sans-serif;font-size:19px;color:white;text-shadow: 1px 1px 1px #000, -1px -1px 1px #000, 1px -1px 1px #000, -1px 1px 1px #000;box-sizing:border-box;user-select:none;background-image: radial-gradient(#48a7d9, #0c6494);z-index: -99;padding: 20px 38px;display: grid;grid-template-columns: repeat(12, 1fr);grid-template-rows: repeat(5, 1fr);grid-column-gap: 0px;grid-row-gap: 0px;')
    freeboard.addStyle('.ip-div1', "grid-area: 1 / 1 / 2 / 5;")
    freeboard.addStyle('.ip-div2', "grid-area: 1 / 5 / 2 / 9; display: flex; justify-content: center;")
    freeboard.addStyle('.ip-div3', "grid-area: 1 / 9 / 2 / 13; display: flex; justify-content: flex-end;")

    freeboard.addStyle('.ip-div4', "grid-area: 2 / 1 / 4 / 5; text-align: center; position: relative; left: -28px; top: 10px;")
    freeboard.addStyle('.ip-div5', "grid-area: 2 / 5 / 4 / 9; text-align: center; z-index: 1;")
    freeboard.addStyle('.ip-font-size-19', "font-size: 19px;")
    freeboard.addStyle('.ip-font-size-26', "font-size: 26px;")
    freeboard.addStyle('.ip-pump', "z-index: -1; position: relative; top: -102px; left: -18px;")
    freeboard.addStyle('.ip-rpm', "line-height: 18px;")
    freeboard.addStyle('.ip-gpm', "line-height: 18px;")
    freeboard.addStyle('.ip-div6', "grid-area: 2 / 9 / 4 / 13; text-align: center; position: relative; left: 29px; top: 10px;")

    freeboard.addStyle('.ip-div7', "grid-area: 4 / 1 / 5 / 5; display: flex; justify-content: center; align-items: center; position: relative; top: 17px;")
    freeboard.addStyle('.ip-div8', "grid-area: 4 / 5 / 5 / 9; display: flex; justify-content: center; align-items: center; position: relative; top: 17px; left: 1px;")
    freeboard.addStyle('.ip-div9', "grid-area: 4 / 9 / 5 / 13; display: flex; justify-content: center; align-items: center; position: relative; top: 17px; left: 1px")
    freeboard.addStyle('.ip-valve-box1', "background-color: rgba(173, 173, 173, 0.5); font-size:15px; width: 95%; height: 75%; display: flex; justify-content: center; align-items: center;")
    freeboard.addStyle('.ip-valve-box2', "background-color: rgba(173, 173, 173, 0.5); font-size:15px; width: 95%; height: 75%; display: flex; justify-content: center; align-items: center;")
    freeboard.addStyle('.ip-valve-box3', "background-color: rgba(173, 173, 173, 0.5); font-size:15px; width: 95%; height: 75%; display: flex; justify-content: center; align-items: center;")

    freeboard.addStyle('.ip-div10', "grid-area: 5 / 1 / 6 / 4; display: flex; align-items: flex-end;")
    freeboard.addStyle('.ip-div11', "grid-area: 5 / 4 / 6 / 7; display: flex; align-items: flex-end; justify-content: center; padding-right: 17px;")
    freeboard.addStyle('.ip-div12', "grid-area: 5 / 7 / 6 / 10; display: flex; align-items: flex-end; justify-content: center; padding-left: 20px;")
    freeboard.addStyle('.ip-div13', 'grid-area: 5 / 10 / 6 / 13; display: flex; align-items: flex-end; justify-content: flex-end;')
    ...
    var mmInfoPanel = function (settings) {
        ...
        this.onCalculatedValueChanged = function (settingName, newValue) {
            ...
            if (settingName == "ip_warning") {
                if (newValue == "1") { document.getElementById('ip-warning-fill').style.fill = 'red' }
                if (newValue == "2") { document.getElementById('ip-warning-fill').style.fill = 'orange' }
                if (newValue == "3") { document.getElementById('ip-warning-fill').style.fill = 'black' }
                if (newValue == "-1") { document.getElementById('ip-warning-fill').style.fill = 'grey' }
            }
            if (settingName == "ip_wifi") {
                if (newValue == "0") { document.getElementById('ip-wifi-fill').style.fill = 'black' }
                if (newValue == "1") { document.getElementById('ip-wifi-fill').style.fill = '#C4DEB5' }
                if (newValue == "-1") { document.getElementById('ip-wifi-fill').style.fill = 'grey' }
            }
            if (settingName == "ip_check") {
                if (newValue == "0") { document.getElementById('ip-check-fill').style.fill = 'red' }
                if (newValue == "1") { document.getElementById('ip-check-fill').style.fill = '#C4DEB5' }
                if (newValue == "-1") { document.getElementById('ip-check-fill').style.fill = 'grey' }
            }
            ...
            if (settingName == "ip_gas_pump") {
                if (newValue == "1") { document.getElementById('ip-fuel-fill').style.fill = '#C4DEB5' }
                if (newValue == "2") { document.getElementById('ip-fuel-fill').style.fill = 'orange' }
                if (newValue == "3") { document.getElementById('ip-fuel-fill').style.fill = 'red' }
                if (newValue == "-1") { document.getElementById('ip-fuel-fill').style.fill = 'grey' }
            }
            if (settingName == "ip_gear") {
                if (newValue == "0") { document.getElementById('ip-gear-fill').style.fill = 'black' }
                if (newValue == "1") { document.getElementById('ip-gear-fill').style.fill = '#C4DEB5' }
                if (newValue == "-1") { document.getElementById('ip-gear-fill').style.fill = 'grey' }
            }
            if (settingName == "ip_gauge") {
                if (newValue == "0") { document.getElementById('ip-gauge-fill').style.fill = 'black' }
                if (newValue == "1") { document.getElementById('ip-gauge-fill').style.fill = '#C4DEB5' }
                if (newValue == "-1") { document.getElementById('ip-gauge-fill').style.fill = 'grey' }
            }
            if (settingName == "ip_watchdog") {
                if (newValue == "0") { document.getElementById('ip-watchdog-fill').style.fill = 'black' }
                if (newValue == "1") { document.getElementById('ip-watchdog-fill').style.fill = '#C4DEB5' }
                if (newValue == "-1") { document.getElementById('ip-watchdog-fill').style.fill = 'grey' }
            }
